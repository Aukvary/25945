...
int main(int argc, char* argv[]) {
    ...
    for (int i = count - 1; i >= 0; i--) {
        ...
        case 'i': {
            printf(
                "UID: real=%d, eff=%d; GID: real=%d, eff=%d\n", (int)getuid(),
        ...
        case 's': {
            if (setpgid(0, 0) == -1) {
                ...
                printf(
                    "Process became group leader. New PGID: %d\n",
                    (int)getpgrp()
                );
            }
            break;
        ...
        case 'u': {
            ...
            if (getrlimit(RLIMIT_NOFILE, &rl) == 0) {
                printf(
                    "ulimit (open files): soft=%f, hard=%f\n",
        ...
        case 'U': {
            ...
            if (*endptr != '\0' || val < 0) {
                fprintf(
                    stderr, "Error: Invalid ulimit value: %s\n", opts[i].arg
                );
            } else {
                struct rlimit rl;
                if (getrlimit(RLIMIT_NOFILE, &rl) == 0) {
        ...
        case 'c': {
            ...
            if (getrlimit(RLIMIT_CORE, &rl) == 0) {
                printf(
                    "Core file size limit: soft=%f bytes, hard=%f bytes\n",
                    (double)rl.rlim_cur, (double)rl.rlim_max
                );
            } else {
                perror("getrlimit failed");
        ...
        case 'C': {
            ...
            if (*endptr != '\0' || val < 0) {
                fprintf(
                    stderr, "Error: Invalid core size value: %s\n", opts[i].arg
                );
            } else {
                struct rlimit rl;
                if (getrlimit(RLIMIT_CORE, &rl) == 0) {
